...
int main(int argc,char **argv)
{
	...
	ros::Subscriber angle_sub = nh.subscribe("/car0/imu/data",10,angleCallback); 
	ros::Publisher vel_pub = nh.advertise<geometry_msgs::Twist>("/car0/cmd_vel",10); 
	...
	while(ros::ok())
	{
		...
		if(command.data == 1)
			{	
				...
				expectPos_car[1] = expectPos_cary_v1;
				expectPos_car[2] = 0;	
				cmd_vel = VelPIDController(car0_pos,uav_pos,expectPos_car); 
				vel_pub.publish(cmd_vel);	
			}	
		if(command.data == 2)
			{		
		...
		ros::spinOnce();
		//cout << "command.data: " << command.data << endl;
		loop_rate.sleep();
